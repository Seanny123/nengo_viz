...
class Converter(object):
    ...
    def process(self, network, id_prefix=None):
        ...
        for i, probe in enumerate(network.probes):
            line = probe._created_line_number-1 # Hopefully this exists?
            is_probing = probe.target # Format information better?
            probe_id = self.namefinder.name(probe)
            # Nodes don't strictly have labels, but it is useful to grab the
    ...
    def to_json(self):
        """ Dump the converted network into JSON"""
